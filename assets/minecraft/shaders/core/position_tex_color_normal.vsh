...
void main() {
    texCoord0 = UV0;
    vertexColor = Color;
    vec3 newPosition = vec3(Position.x, Position.y+(CLOUD_HEIGHT-192.0), Position.z);
    if (abs(Normal.y) > 0.9) {
      if (Position.x < 7.9  || (Position.x < 8.1 && (gl_VertexID % 4 == 1 || gl_VertexID % 4 == 2))) {
        ...
        vertexPosition = IViewRotMat*(ModelViewMat * vec4(newPosition, 1.0)).xyz;
        gl_Position = ProjMat * ModelViewMat * vec4(newPosition, 1.0);
      }
      else if (abs((IViewRotMat*(ModelViewMat * vec4(Position, 1.0)).xyz).y) > 8.0){ // try not to offset if both sides are drawn
        // opposite top/bottom
